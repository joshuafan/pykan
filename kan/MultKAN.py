...
class MultKAN(nn.Module):
    ...
    -----------
        ...
        bias_trainable: bool
            if true, allow subnode_bias to be trainable ONLY
        sp_trainable : bool
    ...
    def __init__(self, width=None, grid=3, k=3, mult_arity = 2, noise_scale=0.3, scale_base_mu=0.0, scale_base_sigma=1.0, base_fun='silu',
                 symbolic_enabled=True, affine_trainable=False, bias_trainable=False, grid_eps=0.02, grid_margin=0.0, grid_range=[-1, 1],
        ...
        for l in range(self.depth):
            ...
            exec(f'self.subnode_bias_{l} = torch.nn.Parameter(torch.zeros(width_out[l+1])).requires_grad_(affine_trainable or bias_trainable)')
    ...
    def update_grid_from_samples(self, x, include_first=True):
        '''
        ...
        -----
            x : 2D torch.tensor
                inputs
            include_first: Whether to update the first layer
        ...
        if include_first:
            ...
            start_layer = 1
        for l in range(start_layer, self.depth):
            self.get_act(x)
    ...
    def plot(self, folder="./figures", beta=3, metric='backward', scale=0.5, tick=False, sample=False, in_vars=None, out_vars=None, title=None, varscale=1.0):
        ...
        for l in range(depth):
            ...
            # Get the actual spline values
            b_splines = B_batch(grid_inputs, self.act_fun[l].grid, k=self.act_fun[l].k)  # [batch, in_dim, G+k]
            weighted_b_splines = b_splines[:, :, None, :] * self.act_fun[l].coef.to(b_splines.device)[None, :, :, :] * self.act_fun[l].scale_sp[None, :, :, None] # [batch, in_dim, out_dim, G+k]
            base_funs = self.act_fun[l].scale_base[None, :, :] * self.act_fun[l].base_fun(grid_inputs)[:, :, None] # [batch, in_dim, out_dim]
            # print("Shape check", grid_inputs.shape, self.act_fun[l].grid.shape, b_splines.shape, self.act_fun[l].coef.shape, weighted_b_splines.shape, base_funs.shape)
            ...
            for i in range(self.width_in[l]):
                ...
                for j in range(self.width_out[l+1]):
                    ...
                    if not torch.allclose(grid_postacts[:, j, i], torch.zeros_like(grid_postacts[:, j, i])):
                        ...
                        # Get individual spline curves
                        plt.plot(grid_preacts[:, j, i].cpu().detach().numpy(), base_funs[:, i, j].cpu().detach().numpy(), color='blue', linestyle='-.', label="Base function")
                        for grid_idx in range(weighted_b_splines.shape[3]):
                            plt.plot(grid_preacts[:, j, i].cpu().detach().numpy(), weighted_b_splines[:, i, j, grid_idx].cpu().detach().numpy(), color='gray', linestyle=':', label="Single spline" if grid_idx == 0 else None)  # spline contribution
    ...
    def reg(self, reg_metric, lamb_l1, lamb_entropy, lamb_coef, lamb_coefdiff, return_indiv=False, flat_entropy=False):
        ...
        if not flat_entropy:
            # Old version: entropy computed per row/column of vec (output/input)
    ...
    def expand_depth(self):
        ...
        self.subnode_bias.append(torch.nn.Parameter(torch.zeros(dim_out,device=self.device)).requires_grad_(self.affine_trainable or self.bias_trainable))
